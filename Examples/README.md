...
`c1i1.1.fastq.gz` `c1i1.2.fastq.gz` `c1i2.1.fastq.gz` `c1i2.2.fastq.gz`
...
for i in c1i1 c1i2
do
bowtie2 -p ${THREADS} -x assembly -1 ${i}.1.fastq.gz -2 ${i}.2.fastq.gz -S ${i}.sam
...
`c1i1.1.fastq.gz` `c1i1.2.fastq.gz` `c1i2.fastq.1.gz` `c1i2.2.fastq.gz`

`c2i1.1.fastq.gz` `c2i1.2.fastq.gz` `c2i2.fastq.1.gz` `c2i2.2.fastq.gz`
...
do
bowtie2 -p ${THREADS} -x metacat.representative -1 ${i}.1.fastq.gz -2 ${i}.2.fastq.gz -S ${i}.sam
samtools sort -l 9 --threads ${THREADS} -o ${i}.bam ${i}.sam
rm ${i}.sam
done
...
--annotation metacat.representative.annotation --bam c1i1.bam c1i2.bam \
--output metacat.variant-c1.tsv
MetaCAT variant --threads-index ${THREADS} --threads-call ${THREADS} \
--annotation metacat.representative.annotation --bam c2i1.bam c2i2.bam \
--output metacat.variant-c2.tsv
rm *.bam.index
```
