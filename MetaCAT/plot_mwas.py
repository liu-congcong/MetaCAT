...
def readVariantAnnotationFile(file):
    ...
    assert openFile.readline().rstrip('\n') == 'Chromosome\tPosition\tAnnotation', f'\"{file}\" is not a valid variant annotation file.'
    for line in openFile:
        lines = line.rstrip('\n').split('\t')
        x[(lines[0], int(lines[1]))] = lines[2]
    openFile.close()
    return x
...
def qq(y, output):
    ...
    figure, axes = subplots(figsize = (3.07, 3.09), squeeze = True)
...
def manhattan(xyz, variantAnnotations, offsets, alpha, output):
    figure, axes = subplots(2, 1, height_ratios = [19, 1], figsize = (6.9, 3), squeeze = False)
...
def main(parameters):
    ...
    qq(xyz[1], parameters.output + '.qq.pdf')
    print(f'{datetime.now().strftime("%Y-%m-%d %H:%M:%S")} -> Creating manhattan plot.', flush = True)
    manhattan(xyz, variantAnnotations, offsets, -log10(parameters.alpha / n), parameters.output + '.manhattan.pdf')
