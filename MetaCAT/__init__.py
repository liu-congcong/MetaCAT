...
from . import (abundance, abundance_test, benchmark_gt, benchmark_rw, c,
               checkm2, cluster, cluster2mapping, color, coverage, gtdbtk,
               index_bam, io_test, mapping2cluster, mwas, plot_mwas,
               representative, seed, variant)


class MetaCATParser(ArgumentParser):
...
def __init__():
    ...
    coverage_parser.add_argument(
        '--mapq', type = int, required = False, metavar = '<int>',
    ...
    coverage_parser.add_argument(
        '--no-index', default = False, action = 'store_true', required = False,
        help = 'Do not create bam index files.\nDefault: False.'
    )
    ...
    seed_parser.add_argument(
        '--temp', default = os.getcwd(), type = str, required = False, metavar = '<str>',
        help = f'Path to the temporary directory.\nDefault: {os.getcwd()}.'
    )
    ...
    cluster_parser.add_argument(
        '--no-fasta', default = False, action = 'store_true', required = False,
        help = 'Do not output fasta formatted files.\nDefault: False.'
    )
    cluster_parser.add_argument(
        '--no-mapping', default = False, action = 'store_true', required = False,
        help = 'Do not output mapping file.\nDefault: False.'
    )
    cluster_parser.add_argument(
    ...
    cluster_parser.add_argument(
        '--min-swdpgmm-n50', default = 1500, type = int, required = False, metavar = '<int>',
        help = 'SWDPGMM is enabled if the N50 is greater than or equal to this value.\nThe value should be a positive integer.\nDefault: 1500.'
    )
    cluster_parser.add_argument(
    ...
    abundance_parser = subparsers.add_parser(
        ...
        usage = '%(prog)s [options] -a <ANNOTATION> -b <BAMs> -o <ABUNDANCE>'
    )
    ...
    abundanceTest_parser.add_argument(
        '--width', default = 2.0, type = float, required = False, metavar = '<float>',
        help = 'Width of a single plot in inches.\nThe value should be a positive float.\nDefault: 2.0'
    )
    abundanceTest_parser.add_argument(
        '--height', default = 1.2, type = float, required = False, metavar = '<float>',
        help = 'Height of a single plot in inches.\nThe value should be a positive float.\nDefault: 1.2'
    )
    ...
    variant_parser = subparsers.add_parser(
        ...
        usage = '%(prog)s [options] -a <ANNOTATION> -b <BAMs> -o <VARIANT>'
    )
...
def find_files(x, suffix):
    for i in x:
        ...
        if i.is_file():
            ...
            for j in i.iterdir():
                if j.is_file() and j.name.endswith(suffix):
                    yield str(j.absolute())
        else:
...
def parse_parameters(parameters):
    ...
    if parameters.command == 'coverage':
        ...
        if parameters.mapq is not None:
        ...
        parameters.fraggenescan = io_test.findExecutablePath(parameters.fraggenescan)
        if parameters.hmmsearch is None:
            parameters.hmmsearch = c.findBinary('hmmsearch')
        parameters.hmmsearch = io_test.findExecutablePath(parameters.hmmsearch)
        parameters.output = os.path.abspath(parameters.output)
        io_test.isWriteable(parameters.output)
        assert parameters.threads > 0, '"-t|--threads" should be a positive integer.'
        parameters.temp = os.path.abspath(parameters.temp)
        io_test.isWriteable(parameters.temp, 'directory')
        ...
        assert parameters.min_swdpgmm_n50 > 0, '"--min-swdpgmm-n50" should be a positive integer.'
        ...
        color.isValidColor(parameters.color)
        ...
        if parameters.engine == 'mash':
            ...
            parameters.mash = io_test.findExecutablePath(parameters.mash)
            ...
            parameters.fastani = io_test.findExecutablePath(parameters.fastani)
            ...
            parameters.skani = io_test.findExecutablePath(parameters.skani)
        ...
        assert parameters.width > 0, '"--width" should be a positive float.'
        assert parameters.height > 0, '"--height" should be a positive float.'
        ...
        io_test.isWriteable(parameters.output)
        assert parameters.threads > 0, '"-t|--threads" should be a positive integer.'
        parameters.checkm2 = io_test.findExecutablePath(parameters.checkm2)
        parameters.temp = os.path.abspath(parameters.temp)
        io_test.isWriteable(parameters.temp, 'directory')
        ...
        if parameters.checkm2 is not None:
            ...
            assert parameters.completeness >= 0 and parameters.completeness <= 1, '"--completeness" should be from 0 to 1.'
        assert parameters.threads > 0, '"-t|--threads" should be a positive integer.'
        parameters.gtdbtk = io_test.findExecutablePath(parameters.gtdbtk)
        parameters.temp = os.path.abspath(parameters.temp)
        io_test.isWriteable(parameters.temp, 'directory')
    return None
