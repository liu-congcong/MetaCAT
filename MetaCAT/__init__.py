...
def __init__():
    ...
    indexBam_parser.add_argument(
        '--input-suffix', default = 'bam', type = str, required = False, metavar = '<str>',
        help = 'If directories are provided with "-i|--input", only files with the specified suffix will be selected.\nDefault: bam.'
    )
    ...
    abundance_parser.add_argument(
        '--mapping-suffix', default = 'mapping', type = str, required = False, metavar = '<str>',
        help = 'If directories are provided with "-m|--mapping", only files with the specified suffix will be selected.\nDefault: mapping.'
    )
    abundance_parser.add_argument(
        '--trim', default = 0, type = int, required = False, metavar = '<int>',
        help = 'Ignore the regions at both ends of a sequence when do calculations.\nThe value should be a non-negative integer.\nDefault: 0.'
    ...
    checkm2_parser.add_argument(
        '--input-suffix', default = 'fasta', type = str, required = False, metavar = '<str>',
        help = 'If directories are provided with "-i|--input", only files with the specified suffix will be selected.\nDefault: fasta.'
    )
...
def parse_parameters(parameters):
    ...
    if parameters.command == 'coverage':
        ...
        files = list()
        for i, file in enumerate(find_files(parameters.input, parameters.input_suffix)):
            files.append(os.path.abspath(file))
            isReadable(files[-1])
        parameters.input.clear()
        parameters.input.extend(files)
        assert parameters.threads > 0, '"-t|--threads" should be a positive integer.'
        ...
        parameters.bam.extend(files)
        files = list()
        for i, file in enumerate(find_files(parameters.mapping, parameters.mapping_suffix)):
            files.append(os.path.abspath(file))
            isReadable(files[-1])
        parameters.mapping.clear()
        parameters.mapping.extend(files)
