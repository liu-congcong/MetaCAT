...
def determineRepresentivesMash(mash, mashP, mashD, mashK, mashS, cluster2file, dpcofg2s2x, temp):
    representives = list()
    ...
    for i, (dpcofg, s2x) in enumerate(dpcofg2s2x.items(), start = 1):
        for s, x in s2x.items():
            if s != 's__':
                X.append(max(x))
                representives.append((cluster2file[X[-1][1]], f'{dpcofg};{s}'))
        if 's__' in s2x:
            offset = len(X)
            s2x['s__'].sort(reverse = True)
            X.extend(s2x['s__'])
            clusterFiles = [cluster2file[i[1]] for i in X]
            tempfile = Path(temp).joinpath(uuid4().hex)
            _mash = Mash(mash, mashD, mashP, mashK, mashS, temp)
            _mash.main(clusterFiles, str(tempfile))
            matrix = readMashFile(clusterFiles, str(tempfile))
            tempfile.unlink()
            ...
            for k in range(offset, y.size):
                if numpy.all(matrix[k, y] == 0):
                    ...
                    representives.append((cluster2file[X[k][1]], f'{dpcofg};s__{g} {j}'))
                    j += 1
        X.clear()
        processBar.plot(i)
...
def determineRepresentivesFastani(fastani, fastaniT, fastaniANI, fastaniK, fastaniFragLen, cluster2file, dpcofg2s2x, temp):
    representives = list()
    processBar = ProcessBar(len(dpcofg2s2x))
    X = list()
    for i, (dpcofg, s2x) in enumerate(dpcofg2s2x.items(), start = 1):
        ...
        if 's__' in s2x:
            ...
            clusterFiles = [cluster2file[i[1]] for i in X]
            tempfile = Path(temp).joinpath(uuid4().hex)
            _fastani = Fastani(fastani, fastaniT, fastaniK, fastaniFragLen, temp)
            _fastani.main(clusterFiles, str(tempfile))
            matrix = readFastaniFile(clusterFiles, fastaniANI, str(tempfile))
            tempfile.unlink()
            y = numpy.zeros(shape = matrix.shape[0], dtype = numpy.bool_)
            y[ : offset] = True
            j = 1
...
def determineRepresentivesSkani(skani, skaniT, skaniANI, skaniC, skaniM, cluster2file, dpcofg2s2x, temp):
    representives = list()
    processBar = ProcessBar(len(dpcofg2s2x))
    X = list()
    for i, (dpcofg, s2x) in enumerate(dpcofg2s2x.items(), start = 1):
        ...
        if 's__' in s2x:
            ...
            clusterFiles = [cluster2file[i[1]] for i in X]
            tempfile = Path(temp).joinpath(uuid4().hex)
            _skani = Skani(skani, skaniT, skaniC, skaniM, temp)
            _skani.dist(clusterFiles, str(tempfile))
            matrix = readSkaniFile(clusterFiles, skaniANI, str(tempfile))
            tempfile.unlink()
            y = numpy.zeros(shape = matrix.shape[0], dtype = numpy.bool_)
            y[ : offset] = True
            j = 1
...
def writeFile(prefix, x):
    ...
    openFile2 = open(f'{prefix}.assembly', 'w')
    for file, classification in x:
        cluster = md5(classification.encode('ascii')).hexdigest()
        openFile1.write(f'{cluster}\t{classification}\n')
        sequenceID = 1
        for _, sequence in readFastaFile(file):
            openFile2.write(f'>{cluster}.{sequenceID}\n')
            openFile2.write(sequence + '\n')
            sequenceID += 1
    openFile1.close()
    openFile2.close()
    return None
...
def main(parameters):
    ...
    if parameters.engine == 'mash':
        representives = determineRepresentivesMash(
            parameters.mash, parameters.threads, 1 - 0.01 * parameters.ani, parameters.mash_kmer_size, parameters.mash_sketch_size, cluster2file, dpcofg2s2x, parameters.temp
        )
    elif parameters.engine == 'fastANI':
        representives = determineRepresentivesFastani(
            parameters.fastani, parameters.threads, parameters.ani, parameters.fastani_kmer_size, parameters.fastani_fragment_length, cluster2file, dpcofg2s2x, parameters.temp
        )
    else:
        representives = determineRepresentivesSkani(
            parameters.skani, parameters.threads, parameters.ani, parameters.skani_compression_factor, parameters.skani_marker_kmer_compression_factor, cluster2file, dpcofg2s2x, parameters.temp
        )
