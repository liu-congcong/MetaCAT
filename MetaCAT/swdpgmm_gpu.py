...
class SWDPGMM:
    ...
    def initializeR(self, y):
        ...
        data = cupy.zeros(shape = self.n, dtype = cupy.int32)
    ...
    def updateMu(self, x, rw):
        ...
        '''
        self.kMu = rw.T.dot(x).astype(cupy.float32) # float64 -> float32 #
    ...
    def updatePrecision(self, x, rw, Sigma):
        ...
        for k in range(self.k):
            ...
            sigma = rw[ : , [k]].multiply(xMinusMu).T.dot(xMinusMu).astype(cupy.float32) # float64 -> float32 #
            sigma += Sigma
    ...
    def main(self, x: NDArray, w: NDArray, y: NDArray) -> NDArray:
        ...
        # init e #
        r = self.initializeR(y) # csr, float64 #
        self.kr = r.sum(axis = 0, dtype = cupy.float32)[0] # float64 -> float32 #
        self.kr += 10 * cupy.finfo(cupy.float32).eps
        rw = r.multiply(w[ : , None]) # float64 #
        rw = rw.tocsc() # csc, float64 #
        self.krw = rw.sum(axis = 0, dtype = cupy.float32)[0] # float64 -> float32 #
        ...
        for i in range(self.maxIterations):
            ...
            self.kr = r.sum(axis = 0, dtype = cupy.float32)[0] # float64 -> float32 #
            self.kr += 10 * cupy.finfo(cupy.float32).eps
            rw = r.multiply(w[ : n, None]) # float64 #
            rw = rw.tocsc() # csc, float64 #
            self.krw = rw.sum(axis = 0, dtype = cupy.float32)[0] # float64 -> float32 #
            self.krw += 10 * cupy.finfo(cupy.float32).eps
