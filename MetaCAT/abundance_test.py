...
def test(x1, x2, coverage, alpha, multipleTest):
    ...
    '''
    p = numpy.ones(shape = x1.shape[0], dtype = numpy.float64)
    q = numpy.ones(shape = x1.shape[0], dtype = numpy.float64)
    ...
    if multipleTest == 'bonferroni':
        ...
        q[mask3] = false_discovery_control(p[mask3], method = 'bh')
    else:
        q[ : ] = p
    numpy.putmask(q, q > 1, 1)
    index = numpy.flatnonzero(q <= alpha)
    return (p, q, index[numpy.argsort(q[index])])


def boxPlot(nodes, rows, columns, x1, x2, p, group1, group2, output, width, height):
    '''
    nodes: list()
    x1: (classifications, individuals)
    x2: (classifications, individuals)
...
def main(parameters):
    ...
    for group1, group2 in combinations(sorted(group2indices.keys()), 2):
        ...
        p, q, index = test(x1, x2, parameters.coverage, parameters.alpha, parameters.multiple_test)
        if index.size:
            ...
            elif rows * columns < index.size:
                print(f'{datetime.now().strftime("%Y-%m-%d %H:%M:%S")} Error: Size of grid must be greater than or equal to the number of plots.', file = stderr, flush = True)
                exit(1)
            boxPlot(
                [nodes[i] for i in index], rows, columns, x1[index], x2[index], p[index],
                group1, group2, f'{parameters.output}.{group1}-{group2}.pdf', parameters.width, parameters.height
            )
        print('Classification', f'Median ({group1})', 'Flag', f'Median ({group2})', 'P', 'Q', sep = '\t')
        for i in zip(nodes, x1, x2, p, q):
            m1 = numpy.median(i[1])
            m2 = numpy.median(i[2])
            if m1 > m2:
                flag = '>'
                ...
                flag = '<'
            if m1 == log10ZeroAbundance or m2 == log10ZeroAbundance:
                if m1 == log10ZeroAbundance:
                    m1 = 'NA'
                if m2 == log10ZeroAbundance:
                    m2 = 'NA'
                print(i[0], m1, '?', m2, 1, 1, sep = '\t', flush = True)
            else:
                print(i[0], m1, flag, m2, i[3], i[4], sep = '\t', flush = True)
