    ...
    for group1, group2 in combinations(sorted(group2indices.keys()), 2):
        ...
        if index.size:
            if parameters.rows is None and parameters.columns is None:
                parameters.columns = ceil(sqrt(index.size))
                parameters.rows = ceil(index.size / parameters.columns)
            elif parameters.rows is None:
                parameters.rows = ceil(index.size / parameters.columns)
            elif parameters.columns is None:
                parameters.columns = ceil(index.size / parameters.rows)
            else:
                assert parameters.rows * parameters.columns >= index.size, 'Size of grid must be greater than or equal to the number of plots.'
            boxPlot(
                [nodes[i] for i in index], parameters.rows, parameters.columns, x1[index], x2[index], p[index],
