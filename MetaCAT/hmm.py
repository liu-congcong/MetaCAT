...
def check_database(hmm: str, n: int = os.cpu_count()) -> tuple[bool, list[str]]:
    ...
    database = os.path.join(os.path.dirname(hmm), md5(os.stat(hmm).st_mtime_ns.to_bytes(16, byteorder = 'little')).hexdigest())
...
def run_hmmpress(hmmpress, hmm):
    ...
    Return:
        None
    '''
    completedProcess = run([hmmpress, '-f', hmm], stdout = DEVNULL, stderr = DEVNULL)
    assert not completedProcess.returncode, 'An error has occured while running hmmpress.'
    return None
...
def hmmsearchProcess(queue, hmmsearch, fastaFile, n, processBar):
    while True:
        ...
        assert not completedProcess.returncode, 'An error has occured while running hmmsearch.'
        n.acquire()
        n.value += 1
...
def createProcesses(hmmsearch, fastaFile, n, threads):
    ...
    for i in range(threads):
        processes.append(Process(target = hmmsearchProcess, args = (queue, hmmsearch, fastaFile, N, processBar)))
        processes[-1].start()
