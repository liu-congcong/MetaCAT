...
def plot(contamination, completenessSet, datasets, programs, program2label, dataset2program2scores, output, rows, columns, colors):
    ...
    figure, axes = subplots(
        ...
        ncols = columns,
        sharey = True,
        figsize = (3.5 * columns, (0.7 + 0.125 * len(programs)) * rows),
        layout = 'constrained',
        squeeze = False
    )
    for i in axes.flat[len(datasets) : ]:
        figure.delaxes(i)
    ...
    for subplot, dataset in zip(axes.flat, datasets):
        ...
        subplot.set_xlabel(f'Number of genomes (Contamination ≤ {contamination:.2f})', fontdict = {'fontsize': 9})
    ...
    figure.legend(
        labels = [f'{i:.2f}' for i in completenessSet],
...
def main(parameters):
    ...
    elif parameters.rows * parameters.columns < nDatasets:
