...
def split_fasta(input_fasta, output_files):
    ...
    magic_code = open4r.read(2)
    open4r.close()
    if magic_code == b'\x1f\x8b':
        decompressed_file = uuid4().hex
        open4r = GzipFile(filename = input_fasta, mode = 'rb')
        open4w = open(decompressed_file, 'wb')
        while open4w.write(open4r.read(10485760)):
            pass
        open4w.close()
        open4r.close()
        input_fasta = decompressed_file
        flag = 1
    else:
        flag = 0
    ...
    file_position = 0
    file_position_ = 0
    open4r = open(input_fasta, mode = 'rb')
