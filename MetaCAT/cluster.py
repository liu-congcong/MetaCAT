...
def output_clusters(sequenceStructs, labels, filePrefix, noFasta, noMapping):
    ...
    if not noFasta:
        for labelID, label in enumerate(uniqueLabels, start = 1):
            ...
            openFile = open(f'{filePrefix}.{labelID:0{labelWidth}d}.fasta', 'w')
            for sequence in sequences:
                openFile.write('>' + sequenceStructs[sequence][0] + '\n')
                openFile.write(sequenceStructs[sequence][1] + '\n')
            openFile.close()
    if not noMapping:
        clusterPrefix = os.path.basename(filePrefix)
        openFile = open(filePrefix + '.mapping', 'w')
        openFile.write('Sequence ID\tCluster ID\n')
        for labelID, label in enumerate(uniqueLabels, start = 1):
            sequences = numpy.flatnonzero(labels == label)
            for sequence in sequences:
                openFile.write(f'{sequenceStructs[sequence][0]}\t{clusterPrefix}.{labelID:0{labelWidth}d}\n')
        openFile.close()
    return None
...
def main(parameters):
    ...
    while processedNSequences < nSequences:
        for label in numpy.unique(labels[labels >= 0]):
            ...
            if recall < 0.2 or numpy.sum(length[sequences]) < parameters.min_cluster: # clusters with recall < 0.2 or size < min_cluster will be removed #
                ...
                flag = True
                if ((nCov > 1 and nClusters >= parameters.min_swdpgmm_clusters) or (sequences.size >= parameters.min_swdpgmm_sequences) or (seeds.size == parameters.max_seeds)) and (n50 >= parameters.min_swdpgmm_n50) and numpy.any(seedLabels):
                    y[ : ] = 0
                    y[seeds] = seedLabels
    ...
    output_clusters(sequenceStructs, labels, parameters.output, parameters.no_fasta, parameters.no_mapping)
