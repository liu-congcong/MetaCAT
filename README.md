...
If you use MetaCAT in your research, please cite:

Liu CC, et al. MetaCAT enables reconstruction of high-quality microbial genomes and their association with host traits from metagenomic data. *Nature Microbiology*. 2026. Accepted for publication.
...
Added the `--bam-suffix` option to the `coverage` command.

Added the `--fasta-suffix` option to the `cluster2mapping` command.

Added the `--input-suffix` option to the `indexBam` command.

Added the `--fasta-suffix` option to the `representative` command.

Added the `--bam-suffix` and `--mapping-suffix` options to the `abundance` command.

Added the `--input-suffix` option to the `variant` command.

Added the `--input-suffix` option to the `checkm2` command.

Added the `--input-suffix` option to the `gtdbtk` command.
...
Added support for `skani`.

Rewrote `indexBam`.

### v1.0.4

Added support for `GTDB-Tk v2.7.1`.

Minor bug fixes.

### v1.0.5

Improved the performance of indexBam.

Added the `--no-index` option to the `coverage` command.

Added the `--no-mapping` option to the `cluster` command.

Renamed `--no-fasta-clusters` to `--no-fasta` in the `cluster` command.

Removed mapping file generation from the `representative` command.

Removed the `--mapping` option from the `abundance` command.

Removed the `--mapping` option from the `variant` command.

Added the `--temp` option to the `checkm2` command.
